...
export default function WrongBook() {
  ...
  const seen = new Set();
  for (const w of list) {
    const key = w.placementId || w.topicId;
    if (!seen.has(key)) {
      seen.add(key);
      topicsInBook.push({ id: key, title: w.topicTitle });
    }
  }

  const shown = filter === 'all' ? list : list.filter((w) => (w.placementId || w.topicId) === filter);
    ...
    <div>
      ...
      <section className="page-panel pink mt12">
        <div className="panel-head">
          <div>
            <h2>错题本</h2>
            <p className="panel-sub">这里收集你做错的题，重点复习这些更高效。做对一道就会自动从错题本移除。</p>
          </div>
          {list.length > 0 && (
        ...
        {list.length === 0 ? (
          ...
          <>
            ...
            <div className="filter-row mt16">
              ...
              {topicsInBook.map((t) => {
                const n = list.filter((w) => (w.placementId || w.topicId) === t.id).length;
                return (
                  <button
                    key={t.id}
            ...
            <div className="mt16" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {shown.map((w) => (
                <div className="wrong-item content-card" key={w.ts}>
